...
process.on('uncaughtException', (error) => {
    ...
    if (error.syscall === 'write' && (error.errno === -4095 || error.errno === -32)) return;
...
class MusicQueue {
    ...
    setupPlayerEvents() {
        ...
        this.player.on('error', error => {
            ...
            this.textChannel.send(`${e("ERROR")} Player error: ${error.message}`).catch(console.error);
            this.processQueue();
        });
    }
    ...
    async play() {
        ...
        try {
            ...
            if (existingPanel?.message) {
                ...
                const storedId = getControllerPanel(this.textChannel.id);
                if (storedId) {
                    try {
                        message = await this.textChannel.messages.fetch(storedId);
                    } catch {}
                }
            }

            const payload = { components: controller.components, flags: controller.flags };
            if (message && typeof message.edit === "function") {
                try {
                    message = await message.edit({ embeds: [], ...payload });
                } catch {
                    message = await this.textChannel.send(payload);
                }
            } else {
                message = await this.textChannel.send(payload);
            }
            ...
            panelClient.musicPanels.set(this.guildId, { message, song, startTime: Date.now() });
            ...
            this.textChannel.send(`${e("ERROR")} Error playing **${song.name}**: ${error.message}`);
            this.songs.shift();
            this.processQueue();
        }
    }
    ...
    stop() {
        ...
        const isInstance = this.client && this.client.INSTANCE_NAME;
        if (!isInstance && this.connection && this.connection.state.status !== VoiceConnectionStatus.Destroyed) {
            try {
...
async function processSpotifyPlaylistBackground(queue, remainingTracks, textChannel) {
        ...
        try {
            ...
            if (youtubeVideo) {
                ...
                if (convertedCount % 10 === 0) {
                    textChannel.send(`${e("MUSIC")} **Spotify Converter**: Added ${convertedCount}/${remainingTracks.length} tracks to queue`).catch(console.error);
                }
            } else {
                failedCount++;
            }
    ...
    if (convertedCount > 0) {
        textChannel.send(`${e("SUCCESS")} **Spotify Converter Complete**: Added ${convertedCount} tracks, ${failedCount} failed`).catch(console.error);
    }
}
...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isChatInputCommand()) {
        ...
        try {
            ...
            const errorMessage = { content: `${e("ERROR")} There was an error executing this command!`, flags: 64 };
...
async function handleButtonInteraction(interaction, client) {
    ...
    if (!queue) {
        return interaction.followUp({ content: `${e("ERROR")} Nothing is playing right now.`, flags: 64 });
    }
    ...
    if (!voiceChannel || voiceChannel.id !== queue.voiceChannel.id) {
        return interaction.followUp({ content: `${e("ERROR")} You need to be in the same voice channel.`, flags: 64 });
    }
    ...
    try {
        switch (interaction.customId) {
            case 'music_pause':
                if (queue.paused) {
                    queue.resume();
                    await interaction.followUp({ content: `${e("PLAY")} Resumed the music.`, flags: 64 });
                } else {
                    queue.pause();
                    await interaction.followUp({ content: `${e("PAUSE")} Paused the music.`, flags: 64 });
                }
                break;

            case 'music_skip':
                queue.skip();
                await interaction.followUp({ content: `${e("SKIP")} Skipped the current song.`, flags: 64 });
                break;

            case 'music_stop':
                queue.stop();
                await interaction.followUp({ content: `${e("STOP")} Stopped the music and cleared the queue.`, flags: 64 });
                break;

            case 'music_shuffle':
                queue.shuffle();
                await interaction.followUp({ content: `${e("SHUFFLE")} Shuffled the queue.`, flags: 64 });
                break;
            ...
            case 'music_loop':
                ...
                await interaction.followUp({ content: `${e("LOOP")} Loop mode: **${modes[nextMode]}**`, flags: 64 });
                break;

            case 'music_previous':
                await interaction.followUp({ content: `${e("PREVIOUS")} Previous track not available.`, flags: 64 });
                break;
            ...
            case 'music_voldown':
                ...
                await interaction.followUp({ content: `${e("VOLDOWN")} Volume: **${newVolDown}%**`, flags: 64 });
                break;

            case 'music_volup':
                const newVolUp = Math.min(100, queue.volume + 10);
                queue.setVolume(newVolUp);
                await interaction.followUp({ content: `${e("VOLUP")} Volume: **${newVolUp}%**`, flags: 64 });
                break;

            case 'music_refresh':
                await interaction.followUp({ content: `${e("REFRESH")} Music controller refreshed!`, flags: 64 });
                break;

            default:
                await interaction.followUp({ content: `${e("WARNING")} Unknown button action.`, flags: 64 });
        }
        ...
        try {
            await interaction.followUp({ content: `${e("ERROR")} An error occurred.`, flags: 64 });
        } catch (replyError) {
            console.error('Failed to send error message:', replyError);
        }
    }
