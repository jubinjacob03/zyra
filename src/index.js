 ...
 * Orchestrates the main bot and multiple instances.
...
const formatLine = (level, label, message, colored) => {
  ...
  const lvl = colored
    ? box(meta.label, meta.fg, meta.bg)
    : meta.label.padEnd(5, " ");
  const scope = label ? (colored ? formatLabel(label) : label) : "";
...
const startProcess = (label, script, args = []) => {
  ...
  log(
    "info",
    label,
    `Starting ${script}${args.length ? " " + args.join(" ") : ""}`,
  );
...
const mainPath = path.join(__dirname, "bot.js");
