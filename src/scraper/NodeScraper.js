...
async function atomicWriteConfig(config) {
  ...
  try {
    ...
    console.error(`❌ [Scraper] Critical failure during atomic config write: ${err.message}`);
    throw err;
  }
}
...
async function runScrapeCycle(watchdog) {
  ...
  try {
    ...
    console.error(`❌ [Scraper] Failed to fetch node list from community API: ${err.message}`);
    return;
  }

  if (!Array.isArray(rawNodes)) {
    console.error("❌ [Scraper] Community API returned invalid JSON structure.");
    return;
  }
  ...
  console.log(`[Scraper] Validated and deduplicated ${deduplicatedNodes.length} nodes. Testing connectivity...`);
  ...
  if (topNodes.length === 0) {
    console.log("⚠️ [Scraper] No alive nodes found during this cycle. Retaining existing configuration.");
    return;
  }
  ...
  try {
    ...
    console.log("💾 [Scraper] Successfully executed atomic write to config.json.");
  } catch (err) {
    console.error(`❌ [Scraper] Failed to update config.json: ${err.message}`);
  }

  if (!watchdog || !watchdog.shoukaku || !watchdog.shoukaku.nodes) {
    console.warn("⚠️ [Scraper] Shoukaku instance not fully initialized. Skipping hot-swap.");
    return;
  }
  ...
  try {
    ...
    console.error(`❌ [Scraper] Failure during dynamic hot-swap: ${err.message}`);
  }
}
