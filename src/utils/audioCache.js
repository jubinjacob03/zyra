    ...
    try {
      ...
      try {
        ...
        if (!oauthToken && process.env.YOUTUBE_OAUTH_TOKEN) {
          ...
          console.log(`✅ Using OAuth token from environment variable`);
        }
      } catch (e) {
...
`;
      ...
      const ytdlpCommands = [
        oauthToken
          ? `yt-dlp -f "bestaudio[ext=webm]/bestaudio/best" --get-url --no-playlist --cookies "${cookiesPath}" --extractor-args "youtube:player_client=android" "${youtubeUrl}"`
          : null,
        oauthToken
      ...
      if (!streamUrl || !streamUrl.startsWith("http")) {
        ...
        if (oauthToken && !fs.existsSync(cookiesPath)) {
          const cookiesContent = `# Netscape HTTP Cookie File
# This file is generated by yt-dlp. Do not edit.
